...
var canvasLibURLArr = ['lib/CanvasRenderer.js', 'lib/Projector.js'];
...
function viewInit() {
  ...
  camera = new THREE.PerspectiveCamera(
    75,
    langasId.clientWidth / langasId.clientHeight,
    1,
    10000
  );
...
function loadObject(url, metaNr, o_name) {
  var onProgress = function (xhr) {
    if (xhr.lengthComputable) {
      ...
      _('wi_info').innerHTML =
        metaNr + 1 + '# ' + Math.round(percentComplete, 2) + '% load';
      view_name = '';
    }
  };
  ...
  if (document.mozFullScreenEnabled) {
    ...
    document.addEventListener('MSFullscreenChange', function () {
      ...
      var loop = setInterval(function () {
        if (
          i > 10 ||
          (window.screen.width == langasId.clientWidth &&
            window.screen.height == langasId.clientHeight)
        ) {
          clearInterval(loop);
          onWindowResize();
        }
